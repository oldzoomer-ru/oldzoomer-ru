# Обо мне

Глубокая экспертиза в разработке на Java (17+) и участием в миграции legacy-системы на более современные технологии.

Знаю, как эффективно проектировать, тестировать и поддерживать надёжные бэкенд-решения с использованием Spring Boot, Spring Data и микросервисного подхода.

Эксперт в вопросах качества кода: регулярно применяю автотестирование (JUnit Jupiter, Mockito), участвую в code review и следую best practices.

Хорошо развиты навыки работы с реляционными БД (Microsoft SQL Server), включая интеграцию с хранимыми процедурами и оптимизацию DAO-слоя.

Умею настраивать и сопровождать DevOps-процессы: сборка через Gradle/Maven, контейнеризация (Docker, docker-compose), CI/CD (GitHub Actions), работа в Linux-среде.

Отличное понимание принципов ООП, чистой архитектуры и современных практик разработки. Опыт с Kafka, Redis и S3 получен в пет-проектах, готов применять их в рабочих задачах.

Интересуюсь инструментами ИИ для разработчиков: пробую локальные LLM и агентные системы для автоматизации рутины и повышения продуктивности.

## Контакты

Email: <work@oldzoomer.ru>

Telegram: <https://t.me/oldzoomer_ru>

Хабр: <https://habr.com/ru/users/oldzoomer/>

## Насчёт FidoNet

Я являюсь одним из самых молодых участников данной legacy-сети в настоящее время, с нодовым адресом [2:5015/519](https://nodehist.wfido.ru/index.cgi?address=2%3A5015%2F519).

Если вы хотите получить поинтовый адрес, [обратитесь ко мне](mailto:fidonet@oldzoomer.ru).

## Спонсорство

Если вы хотите чем-нибудь помочь, вы можете всегда это сделать.

Способы спонсорства:
- Самый лучший подарок - рефка в бигтех :)
- Перевод на карту Альфа-Банка: **2200 1523 0118 3250**
- [Оформить Альфа-Карту](https://alfa.me/3ZcjcH)
- [Инвестировать с Альфа-Инвестициями](https://alfa.me/e-_2kB)
- Записаться к психологу на [Ясно](https://yasno.live/) по промокоду **YASNO-GAVRILOVEGOR** - мне дадут за это бесплатную сессию
